...
├── sqs_lambda                 <-- Source code for a lambda function
│   ├── __init__.py
...
```bash
pip install -r requirements.txt -t sqs_lambda/build/
cp sqs_lambda/*.py sqs_lambda/build/
```
...
```bash
aws s3 mb s3://sqslambda --profile <YOUR PROFILE>
```
